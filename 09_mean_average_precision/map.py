    ...
    for idx, pred in enumerate(predictions):
        for bb in gts[0]['boxes']:
            ...
            if iou > 0.5:
                if pred[1] == 1:
                    tp += 1
        prec = tp / (idx+1)     # precision = true positive / ( true positive + false positive ) --> no way to know if is tp or fp, just sum
        rec = tp / total        # recall = true positive / ( true positive + false negative ) --> if not detected, it's fp
